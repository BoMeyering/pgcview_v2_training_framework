    ...
    if 'device' in conf:
        ...
        if conf.device == 'cuda':
            ...
            if not conf.device in device_str:
                rank_log(conf.local_rank, logger.info, "CUDA Device is not available at this time. Falling back to CPU computation.")
            conf.device = device_str
        elif conf.device not in ['cpu', 'cuda']:
            rank_log(conf.local_rank, logger.info, "Incorrect value set for 'conf.device'. Must be one of ['cpu', 'cuda']. Falling back to 'cpu' computation.")
            conf.device = ":".join(['cpu', str(conf.local_rank)])
    else:
        rank_log(conf.local_rank, logger.info, "No value set for 'conf.training.device'. Setting to 'cpu'. If 'cuda' devices are available, please explicitly pass 'device: cuda' in the configuration YAML.")
        conf.device = ":".join(['cpu', str(conf.local_rank)])